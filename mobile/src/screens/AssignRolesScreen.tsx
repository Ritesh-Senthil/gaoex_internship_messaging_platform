 ...
 * Uses real-time socket events for role assignment updates
 */
...
export default function AssignRolesScreen() {
  ...
  // Store the member's userId for socket event comparison
  ...
  const fetchData = useCallback(async (showRefresh = false) => {
    try {
      ...
      if (memberResponse.success) {
        ...
        memberUserIdRef.current = member.userId; // Store the userId
      ...
      >
        ...
        <View style={styles.checkboxContainer}>
          {isProcessing ? (
            ...
            >
              {hasRole && <Ionicons name="checkmark" size={16} color={colors.white} />}
            </View>
          )}
        </View>
      </TouchableOpacity>
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <Text style={styles.headerTitle}>
              Manage roles for {memberName || 'this member'}
            </Text>
