 ...
 * Uses real-time socket events for role and member updates
 */
...
export default function RoleDetailScreen() {
  ...
  const [editColor, setEditColor] = useState<string>(colors.roleDefault);
  const [editTier, setEditTier] = useState<RoleTier>(2);
  ...
  const fetchRole = useCallback(async (showRefresh = false) => {
    try {
      ...
      if (roleResponse.success) {
        ...
        setEditTier((roleData.tier ?? 2) as RoleTier);
  ...
  // Subscribe to role and member events for real-time updates
  useEffect(() => {
    joinProgram(programId);
    
    const unsubscribe = subscribeToMemberRoleEvents({
      onRoleUpdated: (data: RoleEventData) => {
        if (data.programId !== programId || data.role.id !== roleId) return;
        setRole(prev => {
          if (!prev) return prev;
          return {
            ...prev,
            name: data.role.name,
            color: data.role.color || prev.color,
            tier: data.role.tier as RoleTier,
            permissions: data.role.permissions,
          };
        });
        // Update edit state if not currently editing
        if (!isEditing) {
          setEditName(data.role.name);
          setEditColor(data.role.color || colors.roleDefault);
          setEditTier(data.role.tier as RoleTier);
        }
      },
      onRoleDeleted: (data: RoleDeletedEventData) => {
        if (data.programId !== programId || data.roleId !== roleId) return;
        Alert.alert('Role Deleted', 'This role has been deleted.', [
          { text: 'OK', onPress: () => navigation.goBack() },
        ]);
      },
      onMemberRoleChanged: (data: MemberRoleChangedEventData) => {
        if (data.programId !== programId) return;
        // Check if this role was added or removed from the member
        const wasAdded = data.roles.some(r => r.id === roleId);
        setRole(prev => {
          if (!prev) return prev;
          const hadRole = prev.members?.some(m => m.userId === data.userId);
          if (wasAdded && !hadRole) {
            // Role was added - increment count (member details not available in event)
            return {
              ...prev,
              memberCount: (prev.memberCount || 0) + 1,
            };
          } else if (!wasAdded && hadRole) {
            // Role was removed - decrement count and remove from members list
            return {
              ...prev,
  ...
  }, [programId, roleId, navigation, isEditing]);
  ...
  const getTierColor = (tier: number): string => {
    switch (tier) {
      case 0: return colors.tierOwner;
      case 1: return colors.tierAdmin;
      case 2: return colors.tierMember;
            ...
            <>
              <TouchableOpacity style={styles.cancelButton} onPress={() => {
                ...
                if (role) {
                  ...
                  setEditTier((role.tier ?? 2) as RoleTier);
