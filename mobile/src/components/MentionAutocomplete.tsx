...
export function filterMentionSuggestions(
  ...
  // Filter users - replace spaces with non-breaking space for multi-word names
  const matchingUsers = users
    ...
    .map(u => ({
      ...
      // Use non-breaking space (\u00A0) to keep multi-word names together
      insertText: `@${u.displayName.replace(/ /g, '\u00A0')}`,
    }));
  suggestions.push(...matchingUsers);

  // Filter roles - replace spaces with non-breaking space
